...
"""CONFLUX modules."""
from conflux.config import CONFLUX_DB, BETA_DB_EC_PATH
...
def _p_nu_spectrum(p, isNu, numass = 0):
    """
...
def electron(ebeta, p, numass=0):
    ...
    return _p_nu_spectrum(p, False, numass)
...
def neutrino(enu, p, numass=0):
    ...
    return _p_nu_spectrum(p, True, numass)
...
class BetaBranch(Spectrum):
    ...
    def __init__(self, Z, A, I, Q, E0, sigma_E0, bfrac, sigma_bfrac,
                 ecfrac, sigma_ecfrac,
        ...
        self.bfrac = bfrac
        self.sigma_bfrac = sigma_bfrac
        self.ecfrac = ecfrac
        self.sigma_ecfrac = sigma_ecfrac

        self.fraction = bfrac+ecfrac
        self.sigma_frac = sigma_bfrac+sigma_ecfrac
        ...
        self.cov = {E0:self.sigma_bfrac**2} # Set the covariance diagonal element to the square of the branch fraction uncertainty

    # Display the vital info of branch.
    def Display(self):
        """Display vital branch info."""
        print("Branch E0 = " + str(self.E0)+ "+\-" + str(self.sigma_E0)
            + ", bfrac = " + str(self.bfrac) + "+\-"+str(self.sigma_bfrac)
            + ", ecfrac = " + str(self.ecfrac) + "+\-"+str(self.sigma_ecfrac)
    ...
    def SetCovariance(self, otherBranch, correlation):
        ...
        self.cov[otherBranch.E0] = (self.sigma_bfrac * correlation
                                    * otherBranch.sigma_bfrac)
    ...
    def BetaSpectrum(self, e, nu_spectrum=False):
        ...
        self.Parameters = {
            'Z': self.Z,
            'A': self.A,
            'W0': (self.E0-ELECTRON_MASS_MEV)/ELECTRON_MASS_MEV ,
            'R': getEltonNuclearRadius(self.A) * 1e-15 / NATURAL_LENGTH,
            'L': self.forbiddenness,
            'c': 1.0,
            'b': self.bAc*self.A,
    ...
    def SpectEndpointUncertMC(self, e, nu_spectrum=False, samples = 30):
        ...
        newParameters['W0'] = E0range/ELECTRON_MASS_MEV -2*ELECTRON_MASS_MEV
    ...
    def CalcECSpectrum(self):
        ...
        for shell, xray_edge in xray_edges.items():
            ...
            # calculate the effective fraction of the subshells
            # ignoring higher order subshells  
            if hi_suborbit == 1:
        ...
        # filling the E_nu number into the spectrum
        binwidth = self.xbins[1]-self.xbins[0]
        y = np.zeros(len(self.xbins))
        y_unc = np.zeros(len(self.xbins))
        for i in range(len(E_nu)):
            E0 = E_nu[i]
            I0 = decay_frac[i]
            sigmaI = decay_frac_unc[i]
            if not (0.0 <= E0 <= self.xbins[-1]): 
                continue
            t = (E0 - self.xbins[0]) / binwidth   # position in bins
            i = int(np.floor(t))
            frac = t - i
            if i < 0:
                y[0] += I0
...
class BetaIstp(Spectrum):
    ...
    def EditBranch(self, defaultE0, **kwargs):
        ...
        Keyword Args:
            ...
            sigma_bfrac (float): The uncertainty of fraction.
        ...
        beta_fraction = self.branches[defaultE0].bfrac if branchexist else 0
        sigma_bfrac = self.branches[defaultE0].sigma_bfrac if branchexist else 0
        ec_fraction = self.branches[defaultE0].ecfrac if branchexist else 0
        sigma_ecfrac = self.branches[defaultE0].sigma_ecfrac if branchexist else 0
    ...
    def CalcCovariance(self, GSF=True):
        ...
        totalFrac = 0
        GSFrac = MaxBranch.fraction
        for E0, branch in self.branches.items():
            totalFrac += branch.fraction
        otherFrac = totalFrac-GSFrac

        if GSF == False or MaxBranch.E0 != self.Q or otherFrac == 0:
        ...
        for i, branchi in self.branches.items():
            for j, branchj in self.branches.items():
                if i == self.Q:
                    correlation = -1*branchj.bfrac/otherFrac
    ...
    def SumSpectra(self, nu_spectrum=True, branchErange=[0, 20.]):
        ...
        for E0i, branchi in self.branches.items():
            ...
            si = branchi.spectrum
            bfi = branchi.bfrac
            ui = branchi.uncertainty

            self.spectrum += si * bfi
            self.spectUnc += ui * bfi

            # add EC neutrino if neutrino spectrum is requested
            if nu_spectrum == True:
                ec_i, ec_unc_i = branchi.CalcECSpectrum()
                self.spectrum += ec_i
                self.spectUnc += ec_unc_i
...
class BetaPlusEngine:
    ...
    targetDB: str = BETA_DB_EC_PATH
    """The file name of beta decay data base, configured in conflux.config (BETA_DB_EC_PATH)"""
    xbins: np.ndarray
    ...
    def LoadBetaDB(self, targetDB=BETA_DB_EC_PATH, missingBranch = 3):
        """
        Load default or input betaDB to obtain beta decay informtion. A customed DB must follow the same format as the default DB.
        
        :param targetDB: The file name of beta decay data base, defaults to BETA_DB_EC_PATH
            ...
            if (ZAI in self.inputlist):
                ...
                if len(isotope) < 1:
                    ...
                    for i in range(missingBranch):
                        ...
                        betaIstp.EditBranch(misE0, E0=misE0, beta_fraction=misfrac)
                ...
                for branch in isotope:
                    fracsum += float(branch.attrib['IB']) + float(branch.attrib['IE'])
                ...
                for branch in isotope:
                    ...
                    beta_fraction = float(branch.attrib['IB'])
                    sigma_bfrac = float(branch.attrib['sigma_IB'])
                    ec_fraction = float(branch.attrib['IE'])
                    sigma_ecfrac = float(branch.attrib['sigma_IE'])
                    if fracsum > 1:
                        beta_fraction /= fracsum

                    betaBranch = BetaBranch(Z, A, I, Q, E0, sigma_E0, 
                                            beta_fraction,
                                            sigma_bfrac, 
                                            ec_fraction,
                                            sigma_ecfrac,
