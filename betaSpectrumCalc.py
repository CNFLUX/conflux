...
def integral(nu_spectrum, p, x_low, x_high):
    ...
    if (x_low <= p.thresh and xh >= p.thresh):
        ...
        ergl = 0.0
        ergh = 0.0
        ergl = integrate.quad(function, x_low, p.thresh)
...
def binned_spectrum(nu_spectrum, params, binwidths, result, bins, lower):
    x = lower
    inter = params_t(Z = params.Z+1, A = params.A, e0=params.e0, WM=params.WM, ftype=params.ftype, thresh=params.thresh)
    ...
    norm = integral(nu_spectrum, inter, 0, inter.e0)
    #print(norm)
    for k in range (0, bins):
        result[k] = integral(nu_spectrum, inter, x, x+binwidths)/binwidths/norm
        x+=binwidths
    return 0
...
parser = argparse.ArgumentParser(description='Calculate neutrino and electron spectra')
parser.add_argument('--weak',   type=float, help='Value of weak magentism constant in %%/MeV [default 0.0047]')
parser.add_argument('--bin',    type=float, help='Bin widths in MeV [default 0.1]')
parser.add_argument('--neutrino', action = "store_true", help='Neutrino spectrum [default]')
parser.add_argument('--output', action = "store_true", help='Neutrino spectrum [default]')
parser.add_argument('--electron', action = "store_true", help='Electron spectrum')
parser.add_argument('--lower',  type=float, help='Lower energy range [default b/2]')
parser.add_argument('--forbid', type=int, help='forbideness according to above table')
parser.add_argument('--Q',      type=float, help='Q value of the input isotope')
parser.add_argument('--A',      type=int, help='A number of hte input isotope')
parser.add_argument('--Z',      type=int, help='Z number of the input isotope')

class options:
    def __init__(self, WM, binwidths, neutrino, lower, forbideness):
        self.WM = WM
        self.binwidths=binwidths
        self.neutrino=neutrino
        self.lower=lower
        self.forbideness=forbideness

def main():

    args = parser.parse_args()

    #arguments = options(WM = 0.0047, binwidths = 0.1, neutrino = True, lower = -1.0, forbideness = 0)
    WM = args.weak if args.weak else 0.0047
    binwidths = args.bin if args.bin else 0.1
    nu_spectrum = False if args.neutrino else True
    lower = args.lower if args.lower else -1.0
    forbideness = args.forbid if args.forbid else 0
    if (lower<0):
        lower=binwidths/2.0

    assert(args.Q and args.Z and args.A)

    ps = params_t(A = args.A, Z=args.Z, e0=args.Q, ftype=forbideness, WM=WM)
    bins=int(20.0/binwidths)
    result = [0]*bins

    binned_spectrum(nu_spectrum, ps, binwidths, result, bins, lower)
